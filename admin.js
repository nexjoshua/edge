...
async function loadStudents() {
  ...
  if (profilesRes.error) {
    tbody.innerHTML = `<tr><td colspan="8" class="admin-table-empty">Couldn't load students — ${escapeHtml(profilesRes.error.message)}</td></tr>`;
    return;
  }
...
function renderTable() {
  ...
  if (!rows.length) {
    tbody.innerHTML = `<tr><td colspan="8" class="admin-table-empty">No students found.</td></tr>`;
    return;
  }
