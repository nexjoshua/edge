...
/* ---------- Admin "View as Student" session isolation ---------- */
/* When reached from admin.html's "View as Student" button, this tab is
   opened with a magic-link token already exchanged for a session stored
   under a SEPARATE localStorage key ("edge:impersonation") — see
   admin.js. Using a distinct storageKey here means this student session
   never overwrites the admin's own login, so "Return to Admin" can just
   drop this session and go back without the admin having to sign in
   again. The ?impersonated=1 URL flag is consumed once and remembered in
   sessionStorage so it survives a page reload while this tab stays open. */

const IMPERSONATION_STORAGE_KEY = "edge:impersonation";
const IMPERSONATION_FLAG_KEY = "edge:isImpersonating";

if (new URLSearchParams(window.location.search).has("impersonated")) {
  sessionStorage.setItem(IMPERSONATION_FLAG_KEY, "1");
  window.history.replaceState({}, "", window.location.pathname);
}
const isImpersonating = sessionStorage.getItem(IMPERSONATION_FLAG_KEY) === "1";

const sb = SUPABASE_ENABLED
  ? supabase.createClient(
      window.SUPABASE_URL,
      window.SUPABASE_ANON_KEY,
      isImpersonating ? { auth: { storageKey: IMPERSONATION_STORAGE_KEY } } : undefined
    )
  : null;
...
const READING_ITEMS = [
  ...
  },
  {
    ...
    correct: 1,
    mcOnly: false,
    accepted: ["determined and resilient"],
  },
  {
    melc: "B",
  ...
  },
  {
    ...
    correct: 2,
    mcOnly: false,
    accepted: ["social cultural", "socio-cultural", "sociocultural"],
  },
  {
    melc: "B",
    melcLabel: "Literary Contextual Lenses",
    ...
    correct: 2,
    mcOnly: false,
    accepted: ["how do social class and gender roles affect the characters"],
  },
  {
    melc: "C",
    ...
    correct: 1,
    mcOnly: false,
    accepted: ["it focuses on one main conflict without act breaks"],
  },
  {
    melc: "C",
    melcLabel: "One-Act Play Prewriting",
  ...
  },
  {
    ...
    correct: 2,
    mcOnly: false,
    accepted: ["internal conflict", "man versus self", "man vs self", "self"],
  },
  {
    melc: "C",
    melcLabel: "One-Act Play Prewriting",
    ...
    correct: 2,
    mcOnly: false,
    accepted: ["to understand the characters traits and motivations"],
  },
];
...
const TEST_ITEMS = [
  ...
  },
  {
    ...
    correct: 1,
    mcOnly: false,
    accepted: ["a longing for freedom despite limits"],
  },
  {
    melc: "B",
    melcLabel: "Literary Contextual Lenses",
  ...
  },
  {
    ...
    correct: 1,
    mcOnly: false,
    accepted: ["reader response", "reader-response"],
  },
  {
    melc: "B",
    melcLabel: "Literary Contextual Lenses",
    ...
    correct: 1,
    mcOnly: false,
    accepted: ["the background factors that shape the story"],
  },
  {
    melc: "B",
    melcLabel: "Literary Contextual Lenses",
  ...
  },
  {
    ...
    correct: 2,
    mcOnly: false,
    accepted: ["inciting event", "inciting incident"],
  },
  {
    melc: "C",
    melcLabel: "One-Act Play Prewriting",
  ...
  },
  {
    ...
    correct: 1,
    mcOnly: false,
    accepted: ["external conflict", "man versus man", "man vs man"],
  },
  {
    melc: "C",
    melcLabel: "One-Act Play Prewriting",
    ...
    correct: 2,
    mcOnly: false,
    accepted: ["brainstorming a central conflict"],
  },
];
...
function saveProgress() {
  ...
  // Invalidate the cached leaderboard so this student's updated numbers
  // show up next time they open the Rank tab (see LEADERBOARD section).
  if (typeof leaderboardCache !== "undefined") leaderboardCache = null;
}
...
const readingTypedEl = document.getElementById("readingTyped");
const readingTypedInput = document.getElementById("readingTypedInput");
const readingTypedSubmit = document.getElementById("readingTypedSubmit");
...
/* ---------- Answer mode (Multiple Choice vs Type Answer) ---------- */
/* A per-device preference, applied to both the Learn tab and the Test tab.
   Items flagged mcOnly (a handful of items whose correct answer is a full
   sentence or word list — see melc_content.py) always render as multiple
   choice regardless of this setting, since typing them out isn't a
   reasonable expectation. */

const ANSWER_MODE_KEY = "edge:answerMode";
let answerMode = localStorage.getItem(ANSWER_MODE_KEY) === "typed" ? "typed" : "mc";

function setAnswerMode(mode) {
  answerMode = mode === "typed" ? "typed" : "mc";
  localStorage.setItem(ANSWER_MODE_KEY, answerMode);
  syncAnswerModeSwitches();
  renderReading();
  if (testRunnerEl.style.display !== "none") renderTestQuestion();
}

function syncAnswerModeSwitches() {
  document.querySelectorAll("[data-answer-mode]").forEach((group) => {
    group.querySelectorAll(".segmented-btn").forEach((btn) => {
      btn.classList.toggle("active", btn.dataset.mode === answerMode);
    });
  });
}
...
function isTypedAnswerCorrect(item, typedValue) {
  const norm = normalizeAnswer(typedValue);
  if (!norm) return false;
  return item.accepted.includes(norm);
}

document.querySelectorAll("[data-answer-mode]").forEach((group) => {
  group.querySelectorAll(".segmented-btn").forEach((btn) => {
    btn.addEventListener("click", () => setAnswerMode(btn.dataset.mode));
  });
});
syncAnswerModeSwitches();
...
function renderReading() {
  ...
  wrongAttempts = 0;
  renderReadingAnswerUI(item);
}
renderReading();

function renderReadingAnswerUI(item) {
  const useTyped = answerMode === "typed" && !item.mcOnly;
  readingOptionsEl.style.display = useTyped ? "none" : "flex";
  readingTypedEl.style.display = useTyped ? "block" : "none";

  if (useTyped) {
    readingTypedInput.value = "";
    readingTypedInput.disabled = false;
    readingTypedSubmit.disabled = false;
    setTimeout(() => readingTypedInput.focus(), 50);
  } else {
...
function selectReadingOption(selectedIndex) {
  ...
  handleReadingAnswerResult(isCorrect, item, () => {
    buttons.forEach((b) => {
      b.disabled = false;
...
function submitReadingTyped() {
  const item = currentReadingItem();
  const isCorrect = isTypedAnswerCorrect(item, readingTypedInput.value);
  readingTypedInput.disabled = true;
  readingTypedSubmit.disabled = true;
  readingTypedInput.classList.toggle("correct", isCorrect);
  readingTypedInput.classList.toggle("incorrect", !isCorrect);

  handleReadingAnswerResult(isCorrect, item, () => {
    readingTypedInput.disabled = false;
    readingTypedSubmit.disabled = false;
    readingTypedInput.classList.remove("correct", "incorrect");
    readingTypedInput.value = "";
    readingTypedInput.focus();
  }, item.options[item.correct]);
}

function handleReadingAnswerResult(isCorrect, item, resetFn, correctAnswerText) {
  if (isCorrect) {
    ...
    wrongAttempts += 1;
    readingFeedbackEl.textContent = correctAnswerText
      ? `Not quite — the answer was "${correctAnswerText}". Try again.`
      : "Not quite — check the passage and try again.";
...
readingTypedSubmit.addEventListener("click", submitReadingTyped);
readingTypedInput.addEventListener("keydown", (e) => {
  if (e.key === "Enter" && !readingTypedInput.disabled) submitReadingTyped();
});
...
const testTypedEl = document.getElementById("testTyped");
const testTypedInput = document.getElementById("testTypedInput");
const testTypedSubmit = document.getElementById("testTypedSubmit");
...
function renderTestQuestion() {
  ...
  const useTyped = answerMode === "typed" && !item.mcOnly;
  testOptionsEl.style.display = useTyped ? "none" : "flex";
  testTypedEl.style.display = useTyped ? "block" : "none";

  if (useTyped) {
    testTypedInput.value = "";
    testTypedInput.disabled = false;
    testTypedSubmit.disabled = false;
    testTypedInput.classList.remove("correct", "incorrect");
    setTimeout(() => testTypedInput.focus(), 50);
    return;
  }
...
function advanceTestAfterAnswer() {
  setTimeout(() => {
    testIndex += 1;
...
function selectTestOption(selectedIndex) {
  ...
  advanceTestAfterAnswer();
}

// Unlike the Learn tab, a scored Pre-/Post-Test grades the first attempt —
// no "try again" here, since this is meant to measure what the student
// actually knows right now.
function submitTestTyped() {
  const item = TEST_ITEMS[testIndex];
  const isCorrect = isTypedAnswerCorrect(item, testTypedInput.value);
  testTypedInput.disabled = true;
  testTypedSubmit.disabled = true;
  testTypedInput.classList.add(isCorrect ? "correct" : "incorrect");
  if (isCorrect) testScore += 1;
  advanceTestAfterAnswer();
}

testTypedSubmit.addEventListener("click", submitTestTyped);
testTypedInput.addEventListener("keydown", (e) => {
  if (e.key === "Enter" && !testTypedInput.disabled) submitTestTyped();
});
...
/* ===================== LEADERBOARD ===================== */
/* Reads from the public.leaderboard view (see supabase-admin-addon.sql),
   which only exposes rankable fields (first name + last initial, grade,
   lesson/streak/level) — never email or other private data. Requires
   Supabase to be connected; offline demo mode has no other students to
   compare against, so it shows an explanatory note instead. */

const leaderboardListEl = document.getElementById("leaderboardList");
const leaderboardOfflineNoteEl = document.getElementById("leaderboardOfflineNote");
const scopeAllGradesBtn = document.getElementById("scopeAllGrades");
const scopeMyGradeBtn = document.getElementById("scopeMyGrade");
let leaderboardScope = "all"; // "all" | "mine"
let leaderboardCache = null;

function rankInitials(row) {
  const first = (row.first_name || "S").charAt(0);
  const last = (row.last_initial || "").charAt(0);
  return (first + last).toUpperCase() || "ST";
}

async function renderLeaderboard() {
  if (!SUPABASE_ENABLED || !sb) {
    leaderboardOfflineNoteEl.style.display = "block";
    leaderboardListEl.innerHTML = "";
    return;
  }
  leaderboardOfflineNoteEl.style.display = "none";
  leaderboardListEl.innerHTML = `<p class="leaderboard-empty">Loading rankings…</p>`;

  if (!leaderboardCache) {
    const { data, error } = await sb
      .from("leaderboard")
      .select("user_id, first_name, last_initial, grade_level, lesson, streak, level");
    if (error) {
      leaderboardListEl.innerHTML = `<p class="leaderboard-empty">Couldn't load the leaderboard right now.</p>`;
      console.error("EDGE: leaderboard fetch failed", error);
      return;
    }
    leaderboardCache = data || [];
  }

  let rows = leaderboardCache.slice();
  if (leaderboardScope === "mine" && progress.gradeLevel) {
    rows = rows.filter((r) => String(r.grade_level) === String(progress.gradeLevel));
  }

  rows.sort((a, b) => (b.lesson - a.lesson) || (b.streak - a.streak) || (b.level - a.level));

  if (!rows.length) {
    leaderboardListEl.innerHTML = `<p class="leaderboard-empty">No students to rank yet.</p>`;
    return;
  }
  ...
  leaderboardListEl.innerHTML = rows
    .map((row, i) => {
      ...
      const rankContent = rank <= 3 ? ["🥇", "🥈", "🥉"][rank - 1] : rank;
      const displayName = escapeHtmlLB(`${row.first_name}${row.last_initial ? " " + row.last_initial + "." : ""}`);
      return `
        <div class="leaderboard-row${isMe ? " is-me" : ""}">
          <span class="${rankBadgeClass}">${rankContent}</span>
          <span class="lb-avatar">${rankInitials(row)}</span>
          <div class="lb-name-col">
            <div class="lb-name">${displayName}${isMe ? " (You)" : ""}</div>
            <div class="lb-grade">${row.grade_level ? `Grade ${escapeHtmlLB(String(row.grade_level))}` : "—"}</div>
          </div>
          <div class="lb-stats">
            <div class="lb-stat"><div class="lb-stat-value">${row.lesson}</div><div class="lb-stat-label">Lessons</div></div>
            <div class="lb-stat"><div class="lb-stat-value">${row.streak}</div><div class="lb-stat-label">Streak</div></div>
            <div class="lb-stat"><div class="lb-stat-value">${row.level}</div><div class="lb-stat-label">Level</div></div>
          </div>
        </div>`;
    })
    .join("");
...
scopeAllGradesBtn.addEventListener("click", () => {
  leaderboardScope = "all";
  scopeAllGradesBtn.classList.add("active");
  scopeMyGradeBtn.classList.remove("active");
  renderLeaderboard();
});
scopeMyGradeBtn.addEventListener("click", () => {
  leaderboardScope = "mine";
  scopeMyGradeBtn.classList.add("active");
  scopeAllGradesBtn.classList.remove("active");
  renderLeaderboard();
});

// Progress changes (finishing a lesson, etc.) should invalidate the cache
// so the student sees their own updated rank next time they open the tab.
const _origSaveProgress = saveProgress;
saveProgress = function patchedSaveProgress() {
  leaderboardCache = null;
  return _origSaveProgress.apply(this, arguments);
};

/* ===================== ADMIN IMPERSONATION BANNER ===================== */
/* If this session was reached via the admin dashboard's "View as Student"
   button, app.js was loaded with a separate Supabase client (storageKey
   "edge:impersonation" — see the sb initialization above) that holds the
   student's session without touching the admin's own login. This banner
   is just the "return to admin" escape hatch. */

function initImpersonationBanner() {
  if (!isImpersonating) return;
  const banner = document.createElement("div");
  banner.className = "impersonation-banner";
  banner.innerHTML = `<span>Viewing as a student (Admin mode)</span><button type="button" id="returnToAdminBtn">Return to Admin</button>`;
  document.getElementById("app").prepend(banner);
  document.getElementById("returnToAdminBtn").addEventListener("click", async () => {
    if (SUPABASE_ENABLED && sb) await sb.auth.signOut();
    sessionStorage.removeItem("edge:isImpersonating");
